...
def center_crop(x, crop_h, crop_w,resize_h=64, resize_w=64):
  ...
  #print("this is center_crop mark")
...
def transform(image,input_height,input_width,resize_height=64,resize_width=64,crop=True):
  if crop:
       ...
       im = Image.fromarray(image[j:j+crop_h, i:i+crop_w].astype('uint8'))
  return np.array(im.resize(resize_h, resize_w), Image.BILINEAR)/127.5 - 1.
'''
...
#        j = int(round((h - input_height) / 2.))
#        i = int(round((w - input_width) / 2.))
        im = Image.fromarray(np.uint8(cropped_image[j:j + input_height, i:i + input_width]))
   else:
