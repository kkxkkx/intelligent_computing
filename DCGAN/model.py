...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, crop=True,
    ...
    if self.dataset_name == 'mnist':
      ...
      self.data = glob(data_path)  #获取训练数据目录所有图片
      if len(self.data) == 0:
  ...
  def train(self, config):
    ...
    if could_load:  # 载入之前训练好的模型，计数器在原数值上增加
      counter = checkpoint_counter
      print(" [*] Load SUCCESS")
    else:
      print(" [!] Load failed...")
      os.makedirs('./{}/loss'.format(config.sample_dir))  #创建记录每轮epoch的loss值变化的图表生成目录
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, int(batch_idxs)):
        ...
        d_loss_array[epoch].append(errD_fake+errD_real)
        g_loss_array[epoch].append(errG)
        #plot(idx_array,d_loss_array,label="epoch%d"%epoch,'r-')
        #plot(idx_array,g_loss_array,label="epoch%d"%epoch,'g-')
      ...
      xlabel('idx')
      xticks(np.arange(0,675,1))
      ylabel('loss')
      yticks(np.arange(0,8,0.5))
      plot(idx_array,d_loss_array[epoch],'r-',label='d_loss')
      plot(idx_array,g_loss_array[epoch],'g-',label='g_loss')
      legend(loc='best')
      savefig('./{}/loss/'.format(config.sample_dir)+'%d.png'%(epoch+80),format='png')
#show()
